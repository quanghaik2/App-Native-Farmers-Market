...
export default function EditCategory() {
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (response.ok && data.image_url) {
          setEditingCategory({ ...editingCategory, image_url: data.image_url });
  ...
  const handleUpdateCategory = async () => {
    if (!validateInputs(editingCategory)) return;

    try {
      await callApi(
        `${URL_CONNECT}/api/categories/${editingCategory.id}`,
        {
          method: "PUT",
      ...
      Alert.alert("Lỗi", "Cập nhật danh mục thất bại: " + error.message);
    }
  };

  return (
    <ScrollView className="bg-white pt-4 px-4 pb-24">
      <Text className="text-2xl font-bold text-green-600 mb-4">Sửa danh mục</Text>
      ...
      {imageUploaded && editingCategory.image_url && (
        <View className="mb-4">
          <Text className="text-sm font-bold text-gray-700 mb-1">Ảnh danh mục</Text>
          <Image
            source={{ uri: `${URL_CONNECT}${editingCategory.image_url}` }}
